...
export default function Activities() {
        ...
        const datasUnicas = [
          ...new Set(
            response.data.map((atividade) =>
              new Date(atividade.startTime).toLocaleDateString('pt-BR', {
                weekday: 'long',
                day: '2-digit',
                month: '2-digit',
              })
  ...
  }, [mainActivitiesForSelectedDate, sideActivitiesForSelectedDate, workshopActivitiesForSelectedDate, userSubscriptions]);
  ...
  function subscribeToActivity(activityId) {
    const urlPost = `${import.meta.env.VITE_API_URL}/activities`;
    const configActivities = {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    axios
      .post(urlPost, { activityId }, configActivities)
      .then((response) => {
        // Atualize o estado local para refletir que o usuário está inscrito na atividade.
        setUserSubscriptions((prevSubscriptions) => ({
          ...prevSubscriptions,
          [activityId]: true,
        }));
        // Recarregue as atividades para garantir que os dados estejam atualizados.
        getActivities();
      })
      .catch((err) => alert('Você não pode se inscrever em duas atividades ao mesmo tempo.'));
  }
  ...
  } else if (listTickets.TicketType.isRemote) {
    return (
      <SCContainer>
        <h1>Escolha de atividades</h1>
        <SCCenterContainer>
    ...
    return (
      <HomeContainer>
        ...
        <TransactionsContainer>
          <Ul>
            <Adjust>
              <Statement>Primeiro, filtre pelo dia do evento:</Statement>
              <Choices>
                {listDate.map((data) => (
                  <ListItemContainer key={data} onClick={() => Chosenday(data)} selected={selectedDay === data}>
                    <div>{data}</div>
                  </ListItemContainer>
                ))}
              </Choices>
                ...
                <Box>
                    ...
                    {mainActivitiesForSelectedDate.map((main, index) => (
                      <Event key={index} style={{ height: `${calculateEventHeight(main.startTime, main.endTime)}px` }}>
                        <TitleTime>
                          ...
                          <Time>
                            {`${new Date(main.startTime).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                              timeZone: 'UTC',
                            })} - ${new Date(main.endTime).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                              timeZone: 'UTC',
                            })}`}
                        ...
                        {main.availableSeats - main.occupiedSeats < 1 && (
                          <Vacancies style={{ color: 'red', marginLeft: '5px' }}>
                            <FiXCircle style={{ fontSize: '24px', color: 'red', marginBottom: '5px' }} />
                            <p>esgotado</p>
                          </Vacancies>
                        )}
                        {(main.availableSeats - main.occupiedSeats >= 1 && !main.subscribed) && !userSubscriptions[main.id] && (
                          <Vacancies style={{ color: ' rgba(7, 134, 50, 1)', marginLeft: '5px' }}>
                            <BiLogIn
                              onClick={() => subscribeToActivity(main.id)}
                              style={{ fontSize: '24px', color: 'rgba(7, 134, 50, 1)', cursor: 'pointer' }}
                            />
                            {main.availableSeats - main.occupiedSeats} vagas
                          </Vacancies>
                        )}
                        {(main.subscribed || userSubscriptions[main.id]) && (
                          <Vacancies style={{ color: ' rgba(7, 134, 50, 1)', marginLeft: '5px' }}>
                            <BiCheckCircle style={{ fontSize: '24px', color: 'rgba(7, 134, 50, 1)' }} />
                            Inscrito
                          </Vacancies>
                        )}
                      </Event>
                    ...
                    {sideActivitiesForSelectedDate.map((side, index) => (
                      <Event key={index} style={{ height: `${calculateEventHeight(side.startTime, side.endTime)}px` }}>
                        <TitleTime>
                          ...
                          <Time>
                            {`${new Date(side.startTime).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                              timeZone: 'UTC',
                            })} - ${new Date(side.endTime).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                              timeZone: 'UTC',
                            })}`}
                        ...
                        <Statement7> </Statement7>
                        {side.availableSeats - side.occupiedSeats < 1 && (
                          <Vacancies style={{ color: 'red', marginLeft: '5px' }}>
                            <FiXCircle style={{ fontSize: '24px', color: 'red', marginBottom: '5px' }} />
                            <p>esgotado</p>
                          </Vacancies>
                        )}
                        {(side.availableSeats - side.occupiedSeats >= 1 && !side.subscribed) && !userSubscriptions[side.id] && (
                          <Vacancies style={{ color: ' rgba(7, 134, 50, 1)', marginLeft: '5px' }}>
                            <BiLogIn
                              onClick={() => subscribeToActivity(side.id)}
                              style={{ fontSize: '24px', color: 'rgba(7, 134, 50, 1)', cursor: 'pointer' }}
                            />
                            {side.availableSeats - side.occupiedSeats} vagas
                          </Vacancies>
                        )}
                        {(side.subscribed || userSubscriptions[side.id]) && (
                          <Vacancies style={{ color: ' rgba(7, 134, 50, 1)', marginLeft: '5px' }}>
                            <BiCheckCircle style={{ fontSize: '24px', color: 'rgba(7, 134, 50, 1)' }} />
                            Inscrito
                          </Vacancies>
                  ...
                  <Statement4>
                    {workshopActivitiesForSelectedDate.map((workshop, index) => (
                      <Event
                        key={index}
                        style={{ height: `${calculateEventHeight(workshop.startTime, workshop.endTime)}px` }}
                      >
                        <TitleTime>
                          <Title>{workshop.title}</Title>
                          <Time>
                            {`${new Date(workshop.startTime).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                              timeZone: 'UTC',
                            })} - ${new Date(workshop.endTime).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                              timeZone: 'UTC',
                            })}`}
                        ...
                        <Statement7> </Statement7>
                        {workshop.availableSeats - workshop.occupiedSeats < 1 && (
                          <Vacancies style={{ color: 'red', marginLeft: '5px' }}>
                            <FiXCircle style={{ fontSize: '24px', color: 'red', marginBottom: '5px' }} />
                            <p>esgotado</p>
                          </Vacancies>
                        )}
                        {((workshop.availableSeats - workshop.occupiedSeats >= 1 && !workshop.subscribed) && !userSubscriptions[workshop.id]) && (
                          <Vacancies style={{ color: ' rgba(7, 134, 50, 1)', marginLeft: '5px' }}>
                            <BiLogIn
                              onClick={() => subscribeToActivity(workshop.id)}
                              style={{ fontSize: '24px', color: 'rgba(7, 134, 50, 1)', cursor: 'pointer' }}
                            />
                            {workshop.availableSeats - workshop.occupiedSeats} vagas
                          </Vacancies>
                        )}
                        {(workshop.subscribed || userSubscriptions[workshop.id]) && (
                          <Vacancies style={{ color: ' rgba(7, 134, 50, 1)', marginLeft: '5px' }}>
                            <BiCheckCircle style={{ fontSize: '24px', color: 'rgba(7, 134, 50, 1)' }} />
                            Inscrito
                          </Vacancies>
...
const SCCenterContainer = styled.div`
...
const SCContainerTitle = styled.div`
  width: 411px;
  height: 46px;
  margin-bottom: 35px;
